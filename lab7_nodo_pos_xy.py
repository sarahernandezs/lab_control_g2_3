...
class NodePosition():
	...
	self.xd = 5*np.cos(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1))
	self.yd = 5*np.sin(np.linspace(-2*np.pi,2*np.pi,len(self.t)+1))
    ...
    def controller(self):
        ...
        self.xe = self.xd[self.s] - self.xr
        self.ye = self.yd[self.s] - self.yr
